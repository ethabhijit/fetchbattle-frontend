...
import { getCreatedTournaments } from "../core/helper/coreapicalls";
...
const CreatedTournaments = () => {
  ...
  const preload = () => {
    getCreatedTournaments(user._id).then((data) => {
      if (data.error) {
        setError(data.error);
      } else {
        setValues(data);
  ...
  return (
    <div>
      ...
      <section className="container">
        <h3 className="text-center mt-2">Created Tournaments</h3>
